'''
Preparando todo para reclutar héroes y heroínas para la liga de la justicia, el departamento de HR dispone de una larga lista de justicieros pero solo tiene información detallada
de algunos de ellos.
Es por eso que te piden que desarrolles un pequeño programa el cual basado en la lista 'heroes_para_reclutar' busque en el diccionario '
heroes_info' los que coincidan y los guarde en un nuevo diccionario para luego imprimir por consola todos sus datos. (id, origen, etc)
...
for heroes in heroes_para_reclutar :
    ...
    if heroes in heroes_info.keys():
        info_heroe = heroes_info[heroes]
        id = info_heroe["ID"]
        origen = info_heroe["Origen"]
        Identidad = info_heroe["Identidad"]
        habilidad_lista = info_heroe["Habilidades"]
        habilidades_unicas = set(habilidad_lista)
        habilidad_lista = list (habilidades_unicas)
        habilidades_str = " |  ".join (habilidad_lista)
    
        
        print( "iD : "  , id   , " , codename : " , heroes)
        print("ideantidad : " , Identidad , " , origen : " , origen)
